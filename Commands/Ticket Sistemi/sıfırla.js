const { ChatInputCommandInteraction, SlashCommandBuilder, Client, EmbedBuilder, PermissionFlagsBits, MessageFlags } = require("discord.js");
...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (sıfırla === 'hepsi') {
      ...
      if (!logKanal) {
        return interaction.reply({ content: "Ticket log kanalı ayarlanmamış!", flags: MessageFlags.Ephemeral });
      }
      mzrdb.delete(`mzrlog_${guild.id}`);
      ...
      if (!yetkiliRol) {
        return interaction.reply({ content: "Destek yetkili rolü ayarlanmamış!", flags: MessageFlags.Ephemeral });
      }
      mzrdb.delete(`mzryetkili_${guild.id}`);
      ...
      if (!limitSayı) {
        return interaction.reply({ content: "Limit sayısı ayarlanmamış!", flags: MessageFlags.Ephemeral });
      }
      mzrdb.delete(`mzrlimit_${guild.id}`);
      ...
      if (!mzrKat) {
        return interaction.reply({ content: "Ticket kategorisi zaten ayarlanmamış!", flags: MessageFlags.Ephemeral });
      }
      mzrdb.delete(`mzrkategori_${guild.id}`);
    }
